	...
	<div class="span12">
		<ul class="nav nav-tabs">
			<?php if ($this->Acl->check('Users','index') == true){?>
				<li><?php echo $this->Html->link(__('User Manager'), array('controller' => 'users','action' => 'index')); ?></li>
			<?php } ?>
			<?php if ($this->Acl->check('Groups','index') == true){?>
				<li class="active"><?php echo $this->Html->link(__('Groups'), array('controller' => 'groups','action' => 'index')); ?></li>
			<?php }?>
...
<div class="row-fluid show-grid">
	<?php if ($this->Acl->check('Groups','add') == true){?>
		...
		<table class="table table-striped table-bordered">
			<thead>
				<tr>
					...
					<?php if ($this->Acl->check('Groups','edit') == true || $this->Acl->check('Groups','delete') == true){?>
					<th class="actions" style="width: 130px; text-align: center;"><?php echo __('Actions'); ?>
					</th>
					<?php } ?>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					...
					<?php if ($this->Acl->check('Groups','edit') == true || $this->Acl->check('Groups','delete') == true){?>
					<td style="text-align: center;"><?php if ($this->Acl->check('Groups','edit') == true){?>
						<input type="button" class="btn btn-small btn-primary"
						onclick='formGroup("<?php echo h($group['Group']['id']); ?>","<?php echo h($group['Group']['name']); ?>");'
						value="<?php echo __('Edit'); ?>" /> <?php }?> <?php if ($this->Acl->check('Groups','delete') == true){?>
						<input type="button" class="btn btn-small btn-danger"
						value="<?php echo __('Delete'); ?>"
						onclick='delGroup("<?php echo h($group['Group']['id']); ?>","<?php echo h($group['Group']['name']); ?>")' />
						<?php }?>
					</td>
					<?php } ?>
				</tr>
		...
		$.sModal({
			...
			buttons : [ {
				...
				click : function(id, data) {
					$.post('<?php echo Router::url(array('controller' => 'groups','action' => 'delete')); ?>/'+group_id,{},function(o){
						$('#container').load('<?php echo Router::url(array('controller' => 'groups','action' => 'index')); ?>', function() {
                			$.sModal('close',id);
                		});
                	},'json');
				}
            ...
            buttons:[
                {
                    ...
                    click:function(id){
                    	if (group_id == null){
	                    	$.post('<?php echo Router::url(array('controller' => 'groups','action' => 'add')); ?>',$('#'+id).find('form').serialize(),function(o){
	                    		if (o.error == 0){
		                    		$('#container').load('<?php echo $this->request->webroot; ?>groups', function() {
		                    			$.sModal('close',id);
		                    		});
	                    		}else{
                    		...
                    		$.post('<?php echo Router::url(array('controller' => 'groups','action' => 'edit')); ?>/'+group_id,$('#'+id).find('form').serialize(),function(o){
                    			if (o.error == 0){
	                    			$('#container').load('<?php echo $this->request->webroot; ?>groups', function() {
		                    			$.sModal('close',id);
		                    		});
