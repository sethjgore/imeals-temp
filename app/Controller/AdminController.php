...
class AdminController extends AppController {
	...
	public function isAuthorized($user = null) {
		...
		$userModel = new User();
		$group = new Group();
		$rs = $userModel->find('first',array('conditions'=>array('User.id' => $user['id'])));
		$action = 'controllers';
		...
		if ($authFlag == false && !empty($user)){
			$authFlag = $this->Acl->check(array('User' => array('User.id' => $user['id'])), $action);
		}
		...
		if (!empty($user)){
			...
			$this->request->data['auth_action'] = $this->request->action;
		}
